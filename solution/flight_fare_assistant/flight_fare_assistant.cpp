...
double calculateTotalFare(double baseFare, bool isRoundTrip, TravelClass travelClass, SeatChoice seatChoice,
  ...
  double adjustedFare = baseFare;

  double surcharge = 0.0;

  if (isRoundTrip) {
    surcharge += adjustedFare * 0.8; // add 80% extra for round-trip
  }

  if (travelClass == TravelClass::FirstClass) {
    surcharge += adjustedFare * 0.5; // add 50% extra for first class
  }

  switch (seatChoice) {
  case SeatChoice::Aisle:
  case SeatChoice::Window:
    surcharge += adjustedFare * 0.1; // add 10% extra for aisle/window seats
    break;
  case SeatChoice::Middle:
    surcharge += adjustedFare * 0.05; // add 5% extra for middle seats
    break;
  default:
    break;
  }

  // surcharge applies for weekday bookings between 16:00 and 22:00
  auto inWeekdayBookingSurchargeWindow = [](Day day, int hours, int minutes) {
    const int dayInt = static_cast<int>(day);
    return dayInt >= static_cast<int>(Day::Monday) && dayInt <= static_cast<int>(Day::Friday) &&
           ((hours >= 16 && hours <= 21) || (hours == 22 && minutes == 0));
  };

  // surcharge applies for weekend bookings from 10:00 to 22:00
  auto inWeekendBookingSurchargeWindow = [](Day day, int hours, int minutes) {
    return (day == Day::Saturday || day == Day::Sunday) &&
           ((hours >= 10 && hours <= 21) || (hours == 22 && minutes == 0));
  };

  int hours = std::stoi(bookingTime.substr(0, 2));
  int minutes = std::stoi(bookingTime.substr(3, 2));
  if (inWeekdayBookingSurchargeWindow(bookingDay, hours, minutes)) {
    surcharge += adjustedFare * 0.1; // add 10% booking surcharge
  } else if (inWeekendBookingSurchargeWindow(bookingDay, hours, minutes)) {
    surcharge += adjustedFare * 0.15; // add 15% booking surcharge
  }

  if (flightDay == Day::Saturday || flightDay == Day::Sunday) {
    surcharge += adjustedFare * 0.15; // add 15% surcharge for weekend flights
  }

  if (isRoundTrip) {
    surcharge *= 2;                  // double all surcharges for both legs
    surcharge -= adjustedFare * 0.8; // subtract one round-trip surcharge to prevent duplication
  }
