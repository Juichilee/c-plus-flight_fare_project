...
double calculateTotalFare(double baseFare, bool isRoundTrip, TravelClass travelClass, SeatChoice seatChoice,
  ...
  // Start with the base fare as the foundation
  double totalFare = baseFare;

  // Calculate round-trip surcharge (80% of base if it's a round trip)
  double roundTripSurcharge = isRoundTrip ? 0.8 * baseFare : 0.0;
  totalFare += roundTripSurcharge;

  // Calculate first class surcharge (50% of base if first class)
  double classSurcharge = (travelClass == TravelClass::FirstClass) ? 0.5 * baseFare : 0.0;

  // Calculate seat selection surcharge based on choice
  double seatSurcharge = 0.0;
  if (seatChoice == SeatChoice::Aisle || seatChoice == SeatChoice::Window) {
    seatSurcharge = 0.1 * baseFare;
  } else if (seatChoice == SeatChoice::Middle) {
    seatSurcharge = 0.05 * baseFare;
  }
  // Note: No surcharge for Unassigned

  // Calculate booking-time surcharge
  // First, parse the hour from bookingTime (e.g., "16:00" -> 16)
  int bookingHour = 0;
  std::stringstream timeStream(bookingTime);
  char colon;  // To skip the ':' in "HH:MM"
  timeStream >> bookingHour >> colon;  // Extracts hour and skips colon (ignores minutes as we only need hour)
  
  double bookingSurcharge = 0.0;
  bool isWeekday = (bookingDay >= Day::Monday && bookingDay <= Day::Friday);
  bool isWeekend = (bookingDay == Day::Saturday || bookingDay == Day::Sunday);
  
  if (isWeekday && bookingHour >= 16 && bookingHour <= 22) {
    bookingSurcharge = 0.1 * baseFare;  // 10% for weekday booking window
  } else if (isWeekend && bookingHour >= 10 && bookingHour <= 22) {
    bookingSurcharge = 0.15 * baseFare;  // 15% for weekend booking window
  }

  // Calculate weekend departure surcharge (15% if flight is on Sat/Sun)
  double departureSurcharge = (flightDay == Day::Saturday || flightDay == Day::Sunday) ? 0.15 * baseFare : 0.0;

  // For round trips, double the non-fare-related surcharges
  // (i.e., everything except the round-trip surcharge itself)
  double multiplier = isRoundTrip ? 2.0 : 1.0;
  totalFare += multiplier * (classSurcharge + seatSurcharge + bookingSurcharge + departureSurcharge);
